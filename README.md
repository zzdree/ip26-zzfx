# ⚡ zzfx & zz-suite — Resolume Performance VJ Plugins
**Professional VJ Performance Toolkit untuk Ibadah Perdana UKK UNNES 2026**  
...
## 🌟 Dua Pilihan Arsitektur: Modular Suite vs All-in-One

Project ini menyediakan **dua pendekatan kerja** sesuai kebutuhan operasional VJ saat live performance:

```
┌────────────────────────────────────────────────────────────────────────┐
│ PILIHAN 1: MODULAR SUITE (zz-suite) — REKOMENDASI FLEKSIBEL & RINGAN   │
│  ├─ zz_pusher.cwired    (Elastic Zoom Kick & Beat Punch)               │
│  ├─ zz_chaser.cwired    (7-Direction Dedicated Piano Chaser)           │
│  ├─ zz_strobe.cwired    (High-Speed Multi-Rate Flash)                  │
│  ├─ zz_outliner.cwired  (Neon Sobel Edge Glow Overlay)                 │
│  └─ zz_wiper.cwired     (Linear Scanner Light Beam & Curtain Wipe)     │
│                                                                        │
│  Keuntungan: Bebas drag hanya efek yang dibutuhkan, urutan rack bebas  │
│  diatur vertikal, sangat ringan, hemat GPU & VRAM.                     │
└────────────────────────────────────────────────────────────────────────┘

┌────────────────────────────────────────────────────────────────────────┐
│ PILIHAN 2: ALL-IN-ONE (zzfx) — PRAKTIS 1x DRAG MEGA RACK               │
│  └─ zzfx.cwired         (Semua 5 efek digabung dalam 1 panel inspector)│
│                                                                        │
│  Keuntungan: Cukup 1x drag ke Composition/Layer, langsung dapat Master │
│  Punch combo (Spacebar), 7 Chase Triggers, Strobe, Push, & Outline.    │
└────────────────────────────────────────────────────────────────────────┘
```
...
## 🎛️ Panduan Modul: zz-suite (Modular)

### 1. `zz-pusher` (Zoom Beat Punch)
* **`Push (Toggle / Piano)`:** Saklar zoom punch. Set mode **Piano** untuk ketukan kick drum.
...
1. Drag plugin (Modular atau All-in-One) ke **Composition** (atau layer video).
...
3. Klik tombol parameter di inspector, lalu tekan tombol keyboard yang diinginkan:
   * **`Space`** ➔ `Master Punch` *(pada All-in-One)*
   * **`A`** ➔ `Push` *(pada Pusher atau All-in-One)*
   * **`B`** ➔ `Strobe` *(pada Strobe atau All-in-One)*
   * **`W`** ➔ `Wiper` *(pada Wiper)*
   * **`1` s/d `7`** ➔ `Chase 1` s/d `Chase 7` *(pada Chaser)*
...
6. **Hasil:**
   - **Tahan tombol:** Efek langsung aktif dan looping berulang mengikuti tempo lagu.
